...
if category=="regime":
  ...
  shading = xr.open_dataset("%s/%s_%sano_%s_clim_%s"%(dir_composites, regime, variable, clim_type, season))
  contouring = xr.open_dataset("%s/%s_%sabs_%s"%(dir_composites, regime, "SLP", season))/100
elif category == "dunkelflaute":
  dir_composites = "%s/df_composites"%(dir_out_variables)
  shading = xr.open_dataset("%s/%s_%sano_%s_clim"%(dir_composites, regime, variable, clim_type))
  contouring = xr.open_dataset("%s/%s_%sabs"%(dir_composites, regime, "SLP"))/100
...
if category == 'regime':
  category = '%s_%s'%(category, season)
plt.savefig("%s/plots/%s_%s_%s_%s.pdf"%(dir_composites, variable, regime, category, clim_type), bbox_inches='tight', dpi=600)
plt.clf()
#plt.show()
...
#plt.savefig("%s/plots/%s_%s_%s_%s_colorbar.pdf"%(dir_composites, variable, regime, category, clim_type), bbox_inches='tight')
plt.clf()
#plt.show()
