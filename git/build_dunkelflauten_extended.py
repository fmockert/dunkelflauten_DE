...
def load_startdates(filename):
    ...
    #dataframe = pd.read_csv(snakemake.input.startdates + "/"+ filename + "_wr_DE_0.06_48.txt", header=None)
    dataframe = dataframe.apply(lambda x: pd.to_datetime(x, format='%Y%m%d_%H'))
    return dataframe
    
wr = pd.read_csv(snakemake.input.weatherregime, index_col =[0], parse_dates=[0]).squeeze() ###modified header=None to header=0
