"""Calculates dark lull periods per country for different thresholds/durations."""
...
def dunkelflaute_moving_average(combined_cf, level, window):
    ...
    a DF. This is a hybrid out of `dunkelflaute_mean_average` and 
    `dunkelflaute_rolling_average` and has only one variable (`level`)
    that needs to be set properly.
    """
    combined = combined_cf.rolling(window, min_periods=0, center=True).mean()
    
    #This section is similar to plain, but with a changed center the counting needed to be adapted
