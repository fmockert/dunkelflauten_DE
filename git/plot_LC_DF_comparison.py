...
#crowded part start
  ...
  #Diagonal
  #ax.plot([-50, 10], [50, -10], color='grey', linestyle='dotted')
#  for regime in ['GL', 'EuBL', 'ScBL']:
#    #& (dunkelflauten.start.dt.month.isin([12,1,2]))
#    regime_color = create_colormaps(rgb1=[255,255,255], rgb2=tuple(int(color_dict[regime]['hex'].lstrip('#')[i:i+2], 16) for i in (0, 2, 4)), num=20, name=regime)
#    sns.kdeplot(x=deltat_onset[regime][regime], y= deltat_decay[regime][regime], fill=False, ax=ax, cmap=regime_color, levels=7, alpha=0.8, thresh=0.2)
...
#crowded part end
  ...
  plt.tight_layout()
  if os.path.isdir('../../outputs/plots/LC')==False: os.system('mkdir ../../outputs/plots/LC')
  if mode=='median': 
